...
#ui .rail:has(button[data-tutorial-flash="true"]){z-index:40!important}#ui .rail button[data-tutorial-flash="true"]{position:relative!important;z-index:41!important;outline:6px solid #FFD34E!important;outline-offset:5px!important;border:4px solid #FFF0A0!important;background-color:#9A5510!important;box-shadow:0 0 0 7px #ffcf55cc,0 0 34px 18px #ffd35fdd!important;animation:tutorialBigFlash .72s steps(2,end) infinite!important}#ui .rail button[data-tutorial-flash="true"] .ui-icon{filter:brightness(2.4) drop-shadow(0 0 5px #fff1a0)!important}
...
#left>.tabs{padding-right:50px}#right>.tabs{padding-left:50px}
#left>.tabs button:not(.fold),#right>.tabs button:not(.fold){min-width:0}
#left>.tabs button.fold,#right>.tabs button.fold{position:absolute;top:50%;z-index:5;margin:0}
#left>.tabs button.fold{right:8px;transform:translateY(-50%)}
#right>.tabs button.fold{left:8px;transform:translateY(-50%)}
#left>.tabs button.fold:hover,#right>.tabs button.fold:hover{transform:translateY(-50%)}
...
.sex-mark{display:inline-block;margin-left:5px;font-family:Arial,"Microsoft YaHei",sans-serif;font-size:.58em;font-weight:500;line-height:1;vertical-align:.22em;letter-spacing:0;color:#87684e}.sex-mark.sex-f{color:#a74d73}.sex-mark.sex-m{color:#3e7199}
...
export class UI {
  ...
  constructor(g         ) {
    ...
    this.touchUi = isCoarsePointer();
    this.root.classList.toggle('touch-ui', this.touchUi);
    try {
      const touchMM = window.matchMedia('(pointer: coarse)');
      const applyTouch = () => {
        this.touchUi = isCoarsePointer();
        this.root.classList.toggle('touch-ui', this.touchUi);
        this.render(true);
      };
      if (touchMM.addEventListener) touchMM.addEventListener('change', applyTouch);
    } catch { /* 旧内核可能不支持 pointer 查询 */ }
  ...
  renderRails()       {
    ...
    const campaign = this.g.sim.campaign;
    let flashLeft = '';
    let flashRight = '';
    if (campaign?.mode === 'tutorial' && this.tutorialActive) {
      if (campaign.phase === 'tutorial-build') flashLeft = 'room';
      else if (campaign.phase === 'tutorial-furnish') flashLeft = 'furn';
      else if (campaign.phase === 'first-recruitment') {
        const needsCorridor = !this.g.tavern.rooms.some((room) => room.kind === 'corridor');
        const needsLounge = !this.g.tavern.rooms.some((room) => room.kind === 'lounge');
        flashLeft = needsCorridor || needsLounge ? 'room' : 'furn';
      } else if (campaign.phase === 'recruit') flashRight = 'staff';
    }
    const btn = (side        , tab        , icon        , name        ) =>
      `<button data-act="rail" data-s="${side}" data-v="${tab}" ${(side === 'left' ? flashLeft : flashRight) === tab ? 'data-tutorial-flash="true"' : ''} title="${name}" aria-label="打开${name}面板">${uiIcon(icon)}</button>`;
  ...
  onClick(e       )       {
    ...
    else if (act === 'certinfo') this.openCertificationInfo();
  ...
  render(force         )       {
    ...
    if (s.campaign?.mode === 'tutorial' && s.campaign.phase === 'tutorial-furnish' && this.leftTab === 'furn') {
      ...
      const targetRoom = targetRoomKind ? this.g.tavern.rooms.find((room) => room.kind === targetRoomKind) : null;
      if (targetRoom && (this.g.selection?.kind !== 'room' || this.g.selection.id !== targetRoom.id)) this.g.selection = { kind: 'room', id: targetRoom.id };
    }
  ...
  renderTop()       {
    ...
    const certPending = stars < 5 && e.rep >= nextTh;
    const certLabel = `待完成 ★${stars + 1} 经营认证`;
    const certButton = certPending ? `<button data-act="certinfo" class="hi" title="查看认证条件">${certLabel}</button>` : '';
  ...
  openCertificationInfo() {
    this.topOverflowOpen = false;
    const sim = this.g.sim;
    const cert = sim.previewCertification();
    if (!cert.requirements.length) {
      this.showModal(`<h3>经营认证</h3><div class="card"><b class="good">五星经营认证已全部完成</b><div class="dim">声望与经营条件都已通过，位面评议会不再追加新的星级考核。</div></div><div class="row" style="margin-top:10px"><button data-act="closemodal">关闭</button></div>`);
      return;
    }
    const rows = cert.requirements.map((row) => `<div class="row"><span>${row.met ? '✓' : '○'} ${htmlText(row.label)}</span><span class="${row.met ? 'good' : 'bad'}">${htmlText(String(row.current))} / ${htmlText(String(row.target))}</span></div>`).join('');
    const note = sim.dayActive
      ? '以上为今日实时进度。认证只在打烊时一次性核对，营业中达标不会立刻升星。'
      : '以上为上一营业日或当前累计。开门后人数、评分和设施进度会重新起算。';
    this.showModal(`<h3>★ ${cert.level} 星经营认证</h3>
      <div class="dim">${htmlText(cert.description || STAR_CERTIFICATIONS[cert.level]?.description || '')}</div>
      <div class="card" style="margin-top:10px;border-left-color:${cert.achieved ? '#65A85B' : '#D3A23A'}">${rows}</div>
      <div class="dim" style="margin-top:8px">${note}</div>
      <div class="row" style="margin-top:10px"><button data-act="closemodal">关闭</button></div>`);
  }
  ...
  openFireConfirm(id        )       {
    ...
    this.showModal(`<h3 class="bad">确认解雇 ${renderPersonName(st, 'span')}？</h3>
      ...
      <div class="row" style="margin-top:12px">
        <button data-act="firego" data-v="${st.id}" style="border-color:#B33C4E">确认解雇</button>
  ...
  openSettings()       {
    ...
    this.showModal(`<h3>⚙ 设置</h3>
      ...
      <div class="dim">跟随阶段：按当前驻留世界和营业/打烊/日结自动换曲。点某一首只在本次打开里锁定试听；刷新或重新进游戏会回到跟随阶段。</div>
    ...
    const rerender = (captureInputs = true)       => {
      ...
      <div class="creator-layout">
        <section class="creator-preview">
          ...
          <div class="${aiDesigned ? 'creator-ai-skills' : 'dim'}">${aiDesigned ? '<b>✦ AI 定制能力：</b>' : ''}${SKILL_KEYS.map((key) => `<span class="traitTag" title="创建完成后可在人物详情中查看说明">${htmlText(SKILL_LABEL[key])}</span> ${displayedSkills[key]}`).join(' · ')}</div>`}
