...
export function parseValidateSave(input) {
  ...
  const requestedMode = normalizedData.sim?.campaign?.mode;
  const trustedMode = requestedMode === 'legacy' && verifyLegacyLayoutSeal(normalizedData) ? 'legacy' : requestedMode === 'free' ? 'free' : 'tutorial';
  const strictLoadedTavern = Tavern.load(normalizedData.tavern, { mode: trustedMode === 'legacy' ? 'legacy' : 'tutorial', strict: true });
