...
import { makeWorldName, WORLD_RACE_WEIGHTS } from './world-identities.js';
...
const WORLD_DIALOGUE_VOICE = {
  ...
  neon_ring: {
    arrival: ['{address}，义体还烫着，先给杯冰的，再找个能看见门口雨的位子。', '七码头刚下夜班，雨灌进领口了；有没有不靠墙广告屏的桌？', '今晚不谈公司的事。热汤、冷啤酒，菜单写成人才看得懂的那种。', '肩上这只机械臂是干活用的，不是展品。先让它靠着，别让人围观。', '我们从镜屏区走路过来，靴子全是酸雨。有地方搁外套吗？'],
    wait: ['{address}，队伍看不见就难受。公司已经让人排了一整天。', '雨衣还在滴水，柜台却没人抬头，这比堵车还磨人。', '招牌写着“即来即坐”，我们站这儿都能把烟抽完一根了。', '义体关节开始响，是真的冷，不是在催你们表演效率。', '给个准话就行：还要几桌，还是今晚没位。'],
    good: ['这碗面是热的，酒是冰的，雨被关在门外。今晚值回那点信用点。', '{address}没拿我们当扫描件，也没追问义体编号，这才像待客。', '真菜叶子，不是合成糊。七码头那帮人听了会骂我乱花钱。', '街机厅不闪灯、不催人，酒也不兑水，难得有地方让人坐得住。', '{self}会跟巷口的人说这儿能歇脚，不卖给董事会那种宣传。'],
    neutral: ['能吃能喝，只是还像公司食堂多加了霓虹灯。', '位子有了，味道也过得去，可雨声比店里热闹。', '没被赶，也没被记住。对环城来说已经算中等偏上。', '酒凑合，灯太亮，像还在加班。', '下次要是能把广告声关小点，评价能再抬一截。'],
    bad: ['{address}，让人干等还把热菜端凉，这跟公司窗口有什么两样。', '标着天然，咬下去是合成粉。这种骗法在下层街会被砸摊。', '义体进门就被当安检对象，酒还没上，火气先上来了。', '账单比菜单贵一截，解释却是“系统自动加收”。少来这套。', '{self}会把今晚写在码头墙上，公关删不掉水泥字。'],
    journey: ['七码头有人出钱买一整条街的停电三分钟，我来问谁还肯接这种活。', '公司把位面门做成会员项目后，下层只能从货运口往外挤。', '老磁带说这儿能听到没被压缩过的歌，我带着一盒母带换路费。', '月轨电梯封了，我们顺着废弃物流道走到这扇门。', '有人悬赏一颗没打过蜡的真苹果。我来看看异界还剩多少土味。'],
  },
  moonsea: {
...
const WORLD_DETAILS = {
  ...
  neon_ring: {
    identity: { environment: '环形巨城、酸雨高架、下层巷口面摊与永不熄灯的广告墙', civilization: '公司占上层、街区自治下层，人与改装机械体挤在同一条雨里', technology: '义体改造、企业监控、黑市芯片和还能用手修的老机器' },
    culture: { values: ['痛快', '不装', '看得见'], hospitalityIdeal: '队伍看得见、热食上得快、把人当街坊而不是工号', taboos: ['让人干等还不给说法', '拿义体和编号当安检借口'], speechStyle: '短句、巷口音，爱抱怨公司、酸雨和信用点，不拿网络术语当日常说话', addressForms: { peer: '老板', formal: '掌柜', elder: '小朋友', notable: '码头店主' }, selfReferences: ['我', '咱', '老子'], socialStrata: ['合同工', '街民', '企业编制'] },
  ...
  moonsea: {
    identity: { environment: '月光海沟、珊瑚浮岛与潮汐驱动的水下城市', civilization: '海国宫廷、鱼人港邦与珊瑚灵巫组成的水域国家', technology: '潮汐工程、水下炼金与声呐魔法' },
...
export const SKILL_HELP = Object.freeze({
  looks: {
    name: '颜值',
    summary: '仪态、第一印象和公开场合的体面程度。',
    effects: [
      '迎宾岗的主能力；前台、迎宾的岗位评分会重点看这项。',
      '调酒岗也有一定权重：吧台形象会影响客人观感。',
      '迎宾、仪态和演出类设施照看等动作，会随数值变快。',
      '干活会攒经验；加点上限 100，装备和技能还能继续提高实效。',
    ],
    jobs: '迎宾（主）、前台、调酒（辅）',
  },
  cook: {
    name: '厨艺',
    summary: '判断食材、火候和异界口味，决定餐食能不能做、好不好吃。',
    effects: [
      '厨师岗的主能力，也是备餐、下锅的主检定。',
      '出品质量随厨艺提高；高门槛菜做不出来会显示“厨艺不足”。',
      '烹饪动作更快；研发新菜时，厨艺越贴近门槛，成功率越高。加点上限 100，装备和技能可继续提高实效。',
      '清洁和冷静会作为厨师岗的次要评分。',
    ],
    jobs: '厨师（主）',
  },
  mix: {
    name: '调酒',
    summary: '风味配比、出杯节奏和对酒客的观察。',
    effects: [
      '调酒师岗的主能力；酒廊和酒吧的调酒都看这项。',
      '决定饮品出品质量和调酒速度。',
      '酒吧调酒比同品质酒廊更快；缺门槛时饮品同样做不出来。',
    ],
    jobs: '调酒（主）',
  },
  serve: {
    name: '服务',
    summary: '迎送、点单、桌边判断和结账时的沟通。',
    effects: [
      '前台、跑堂的主能力；迎宾、带位、点单、上菜、结账都用得到。',
      '相关动作速度随服务提高；好感还会再给一层速度加成。',
      '前台评分里服务权重大于颜值。',
    ],
    jobs: '前台（主）、跑堂（主）、迎宾（辅）',
  },
  clean: {
    name: '清洁',
    summary: '卫生判断、整理次序和设施复位。',
    effects: [
      '清洁岗的主能力；打扫房间、收尾和整理都看这项。',
      '清洁动作更快；洁癖性格还会再加速清洁。',
      '厨师、场务、搬运岗也会把清洁当作次要能力。',
    ],
    jobs: '清洁（主）、厨师/场务/搬运（辅）',
  },
  carry: {
    name: '搬运',
    summary: '负重、路线和安全交接，也影响走路快慢。',
    effects: [
      '搬运岗的主能力；取料、上菜、收脏盘、搬动物资都用这项。',
      '40 为默认移速：每高 1 点走路快 1%，每低 1 点慢 1%。',
      '场务和清洁岗也会看搬运；设施里的搬动、抢修常用这项检定。',
    ],
    jobs: '搬运（主）、场务、清洁、跑堂（辅）',
  },
  calm: {
    name: '冷静',
    summary: '风险识别、投诉应对和高压下的判断。',
    effects: [
      '场务岗的主能力；照看设施、处理事故和多数事件检定都看这项。',
      '前台、跑堂、厨师、调酒的岗位评分都包含冷静。',
      '数值越高，高压动作更稳，事件成功率也更高。',
    ],
    jobs: '场务（主）、前台/迎宾/跑堂/厨师/调酒（辅）',
  },
});

export const NEED_HELP = Object.freeze({
  stamina: {
    name: '体力',
    summary: '还能撑多久。干活会消耗，休息和打烊会恢复。',
    effects: [
      '工作时持续下降；低于 30 走路变慢，低于 25 操作也变慢。',
      '过低会去休息，前台体力不够时会先回自己的休息室。',
      '打烊后店主和员工的体力都会回满。',
    ],
  },
  hunger: {
    name: '饥饿',
    summary: '工作时会慢慢饿；打烊吃饭后下降。',
    effects: [
      '营业中随时间上升；数值越高越饿。',
      '打烊会吃饭，饥饿大幅下降。',
      '打烊偷闲时偷吃、喝茶也能缓解饥饿。',
  ...
  stress: {
    name: '压力',
    summary: '现场积压、体力不足和性格都会把它推高。',
    effects: [
      '客人排队等待、体力过低时压力上升更快。',
      '暴躁性格加压更快；打烊结算时压力会拉低士气。',
      '休息、聊天和部分打烊活动可以减压。',
    ],
  },
  morale: {
    name: '士气',
    summary: '愿不愿意继续认真干活，也影响和店主的关系。',
    effects: [
      '士气高时好感更容易涨；过低则可能掉好感，员工甚至会考虑离开。',
      '欠薪、高压会打击士气；公平待遇、休息和进修能提振。',
    ],
  },
  aff: {
    name: '好感',
    summary: '和店主的亲近程度，影响互动和干活速度。',
    effects: [
      '当前动作速度有好感加成：大约为好感/4%。',
      '聊天、关心和共同经历会增加好感；冷却结束后才能再聊。',
      '高好感会解锁更深入的互动；店主自己没有这项关系需求。',
    ],
  },
});
...
export function makeName(rng, world, opts = {}) {
  return makeWorldName(rng, world, opts);
}
